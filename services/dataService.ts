...
export async function getProspects(userId: string): Promise<Prospect[]> {
    const { data, error } = await (supabase as any).from('prospects').select('*');
    if (error) {
...
export async function addProspects(userId: string, prospects: Omit<Prospect, 'id'| 'user_id'>[]): Promise<Prospect[]> {
    const prospectsWithUserId = prospects.map(p => ({ ...p, user_id: userId }));
    const { data, error } = await (supabase as any).from('prospects').insert(prospectsWithUserId);
