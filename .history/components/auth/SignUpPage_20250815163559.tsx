...
export const SignUpPage: React.FC<SignUpPageProps> = ({ onNavigate }) => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    // normaliza o whatsapp (só dígitos) para salvar limpo no metadata
    const whatsappDigits = whatsapp.replace(/\D/g, '');
    const nameTrimmed = fullName.trim();

    setError(null);

    // 1) cria a conta já com metadata correto (name/whatsapp)
    const { error } = await signUp(email, password, {
      name: nameTrimmed,
      whatsapp: whatsappDigits, // <- chave precisa ser exatamente 'whatsapp'
    });
    ...
    // 2) fallback para garantir persistência do metadata (alguns fluxos descartam data do signUp)
    try {
      ...
      <AuthLayout title="Sucesso!" subtitle="Sua conta foi criada.">
        <div className="text-center">
          ...
          <p className="text-gray-300 mb-6">
            Seu cadastro foi realizado com sucesso. Para sua segurança, enviamos um e-mail de confirmação
            (simulado). Agora você já pode fazer login.
          </p>
          <button
  ...
  return (
    <AuthLayout title="Crie sua Conta" subtitle="Comece a prospectar em minutos.">
      <form onSubmit={handleSubmit} className="space-y-4 mt-4">
        ...
        {(error || authError) && (
          <p className="text-sm text-red-400 text-center pt-2">{error || authError?.message}</p>
        )}
