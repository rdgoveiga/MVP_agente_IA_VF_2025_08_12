...
export const ProspectingTool: React.FC = () => {
  ...
  // >>>>>>> FIX: useEffect correto, sem useCallback interno e com parênteses/chaves fechando certinho
  useEffect(() => {
    if (!user) return;
  ...
  const executeProspecting = useCallback(async () => {
    ...
    try {
      ...
      if (!foundProspects || !Array.isArray(foundProspects)) {
        throw new Error('A resposta da IA não contém prospects válidos.');
      }
      ...
      if (foundProspects.length === 0) {
        ...
        const prospectsWithUserAndSource = foundProspects.map(p => ({ ...p, user_id: user.id, foundOn: Array.from(searchSources) }));
