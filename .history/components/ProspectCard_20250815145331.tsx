...
import { BuildingOfficeIcon, GlobeAltIcon, WhatsAppIcon, CheckCircleIcon, XCircleIcon, InstagramIcon, InformationCircleIcon, ChevronDownIcon, SparklesIcon, DocumentDuplicateIcon, ArrowRightIcon, TrashIcon } from './icons';
...
const AIScoreIndicator: React.FC<{ score: number, "data-tour"?: string }> = ({ score, "data-tour": dataTour }) => {
    const getScoreConfig = () => {
        if (score >= 75) {
            return {
                ...
                explanation: 'Prioridade Alta: Muitas oportunidades de melhoria. Alvo com grande potencial de venda.',
            };
        }
        if (score >= 50) {
            return {
    ...
    return (
        <div title={config.explanation} data-tour={dataTour} className={`flex items-center gap-2 text-xs font-semibold py-1 px-3 rounded-full border ${config.color}`}>
...
export const ProspectCard: React.FC<ProspectCardProps> = ({ prospect, messageTemplate, onUpdateProspect, onDeleteProspect, onShowToast, isFirstCard }) => {
  ...
  const handleWhatsAppClick = async (phone: string, name: string) => {
    // A mensagem inicial para o WhatsApp será a saudação gerada pela IA, se existir.
    // Caso contrário, usa uma saudação genérica.
  ...
  const statusIndicator = () => {
    switch (prospect.status) {
        case 'contacted':
            return (
                <div className="flex items-center gap-2 text-green-400 cursor-default" title="Este prospect já foi contatado.">
        ...
        case 'negotiating':
             return (
                <div className="flex items-center gap-2 text-yellow-400 cursor-default" title="Este prospect está em negociação.">
        ...
        case 'won':
            return (
                <div className="flex items-center gap-2 text-cyan-400 cursor-default" title="Este prospect foi ganho.">
                    <CheckCircleIcon className="h-5 w-5 flex-shrink-0" />
  ...
  return (
    <>
      <div data-tour={isFirstCard ? "prospect-card" : undefined} className="bg-gray-800/50 backdrop-blur-lg border border-gray-700/80 rounded-2xl p-5 flex flex-col h-full transform transition-all duration-300 shadow-lg hover:shadow-blue-500/10 hover:-translate-y-1">
        <div className="flex-grow">
          ...
          <div className="mt-4 bg-gray-900/40 border border-gray-700/60 rounded-lg p-3">
              <div className="flex justify-between items-center mb-2">
                  ...
                  <AIScoreIndicator score={prospect.aiScore} data-tour={isFirstCard ? "prospect-card-aiscore" : undefined} />
              </div>
              <p className="text-sm text-gray-400 italic" data-tour={isFirstCard ? "prospect-card-analysis" : undefined}>"{prospect.analysis}"</p>
              ...
              {prospect.nextRecommendedAction && (
                   <div className="mt-3 pt-3 border-t border-gray-700/50 flex items-center gap-2 text-cyan-300">
                      ...
                      <p className="text-sm font-semibold">
                          Próxima Ação: <span className="font-normal text-cyan-200">{prospect.nextRecommendedAction}</span>
                      </p>
                  </div>
              )}
              ...
              {prospect.analysisBreakdown && prospect.analysisBreakdown.length > 0 && (
                  <div className="mt-3" data-tour={isFirstCard ? "prospect-card-analysis-breakdown" : undefined}>
                      ...
                      >
                          ...
                          <ChevronDownIcon className={`h-5 w-5 transition-transform duration-300 ${isAnalysisExpanded ? 'rotate-180' : ''}`} />
                      </button>
                      {isAnalysisExpanded && (
                          <div className="mt-3 border-t border-gray-700/50 pt-3 flex flex-col gap-3 animate-fade-in-up" style={{ animationDuration: '0.3s' }}>
               ...
               {prospect.improvementSuggestions && (
                  <div className="mt-4 border-t border-gray-700/50 pt-3" data-tour={isFirstCard ? "prospect-card-suggestions" : undefined}>
                      <button onClick={() => setIsSuggestionsExpanded(!isSuggestionsExpanded)} className="flex justify-between items-center w-full text-left">
                          ...
                          <div className="flex items-center gap-2">
                              <button 
                                  onClick={(e) => { e.stopPropagation(); handleCopy(prospect.improvementSuggestions!, 'Sugestões copiadas!'); }}
                              ...
                              <ChevronDownIcon className={`h-5 w-5 text-gray-400 transition-transform duration-300 ${isSuggestionsExpanded ? 'rotate-180' : ''}`} />
                          </div>
                      </button>
                      {isSuggestionsExpanded && (
                         <div className="mt-2 text-sm text-gray-400 bg-gray-900/50 p-2 rounded-md whitespace-pre-wrap font-mono animate-fade-in-up" style={{ animationDuration: '0.3s' }}>{prospect.improvementSuggestions}</div>
                      )}
                  </div>
              )}
          </div>
        ...
        <div className="mt-auto pt-4">
           {hasVisibleLinks && (
              <div className="flex flex-col gap-2">
                  ...
                  {prospect.instagramUrl && prospect.foundOn?.includes('instagram') && (
                    <a
                      href={typeof prospect.instagramUrl === 'string' && prospect.instagramUrl.startsWith('http') ? prospect.instagramUrl : `https://instagram.com/${(prospect.instagramUrl || '').replace('@','')}`}
                      target="_blank"
                      rel="noopener noreferrer"
                    ...
                    >
                      ...
                      <span className="group-hover:underline truncate text-sm" title={getSocialUsername(prospect.instagramUrl) || prospect.instagramUrl}>
          ...
          <div className={`flex flex-col gap-3 ${hasVisibleLinks ? 'pt-3 mt-3 border-t border-gray-700/50' : ''}`}>
              ...
              {generatedMessages && (
                  <div className="bg-blue-900/30 border border-blue-500/40 rounded-lg">
                      <button onClick={() => setIsMessageExpanded(!isMessageExpanded)} className="w-full flex justify-between items-center p-3 text-left">
                          <p className="text-xs font-bold text-blue-300">Sequência de Mensagens Sugerida</p>
                          <ChevronDownIcon className={`h-5 w-5 text-blue-300 transition-transform ${isMessageExpanded ? 'rotate-180' : ''}`} />
                      </button>
                      {isMessageExpanded && (
                         <div className="px-3 pb-3 space-y-3 animate-fade-in-up" style={{ animationDuration: '0.3s' }}>
                              <div className="space-y-1">
                                  <div className="flex justify-between items-center">
                                      ...
                                      <button onClick={() => handleCopy(generatedMessages.greeting, "Saudação copiada!")}><DocumentDuplicateIcon className="h-4 w-4 text-gray-400 hover:text-white"/></button>
                                  </div>
                                  <p className="text-sm text-gray-200 bg-gray-900/30 p-2 rounded-md">{generatedMessages.greeting}</p>
                              </div>
                              <div className="space-y-1">
                                  <div className="flex justify-between items-center">
                                      <label className="text-xs font-semibold text-gray-300">2. Mensagem Principal</label>
                                      <button onClick={() => handleCopy(generatedMessages.mainMessage, "Mensagem principal copiada!")}><DocumentDuplicateIcon className="h-4 w-4 text-gray-400 hover:text-white"/></button>
                                  </div>
                                  <p className="text-sm text-gray-200 bg-gray-900/30 p-2 rounded-md">{generatedMessages.mainMessage}</p>
                              </div>
                              <div className="space-y-1">
                                  <div className="flex justify-between items-center">
                                      <label className="text-xs font-semibold text-gray-300">3. Fechamento</label>
                                      <button onClick={() => handleCopy(generatedMessages.closingMessage, "Fechamento copiada!")}><DocumentDuplicateIcon className="h-4 w-4 text-gray-400 hover:text-white"/></button>
                                  </div>
                                  <p className="text-sm text-gray-200 bg-gray-900/30 p-2 rounded-md">{generatedMessages.closingMessage}</p>
                              </div>
                         </div>
                      )}
                  </div>
              ...
              {prospect.phone ? (
                  ...
                  <div className="text-center text-sm text-gray-500 bg-gray-900/30 border border-dashed border-gray-700 rounded-lg py-3 px-2">
    ...
    <div className="bg-gray-800/50 backdrop-blur-lg border border-gray-700/80 rounded-2xl p-5 flex flex-col h-full">
      <div className="flex-grow">
        <div className="flex items-start gap-4 mb-4">
          <div className="flex-shrink-0 bg-gray-700/80 p-3 rounded-full h-12 w-12 skeleton-loading"></div>
          <div className="w-full">
            <div className="h-6 w-3/4 rounded-md bg-gray-700 skeleton-loading"></div>
            <div className="h-4 w-1/2 rounded-md bg-gray-700 skeleton-loading mt-2"></div>
          </div>
        </div>
        
        <div className="h-4 w-full rounded-md bg-gray-700 skeleton-loading"></div>
        <div className="h-4 w-5/6 rounded-md bg-gray-700 skeleton-loading mt-2"></div>

        <div className="mt-5 bg-gray-900/40 border border-gray-700/60 rounded-lg p-3 space-y-4">
            <div className="flex justify-between items-center">
                <div className="h-4 w-1/4 rounded-md bg-gray-700 skeleton-loading"></div>
                <div className="h-6 w-24 rounded-full bg-gray-700 skeleton-loading"></div>
            </div>
            <div className="h-4 w-11/12 rounded-md bg-gray-700 skeleton-loading"></div>
             <div className="h-4 w-2/3 rounded-md bg-gray-700 skeleton-loading"></div>
        </div>
      </div>
      
      <div className="mt-auto pt-4 space-y-3">
         <div className="h-5 w-1/2 rounded-md bg-gray-700 skeleton-loading"></div>
         <div className="h-5 w-1/3 rounded-md bg-gray-700 skeleton-loading"></div>
         <div className="flex flex-col gap-3 pt-3 mt-3 border-t border-gray-700/50">
            <div className="h-10 w-full rounded-lg bg-gray-700 skeleton-loading"></div>
            <div className="h-10 w-full rounded-lg bg-gray-700 skeleton-loading"></div>
         </div>
      </div>
    </div>
