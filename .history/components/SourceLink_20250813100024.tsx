...
export const ProspectingTool: React.FC = () => {
  ...
  const [settings, setSettings] = useState<UserSettings>({ userId: '', messageTemplate: '', searchSources: [], includeWhatsApp: false });
  ...
  /** Carrega dados do usuário (prospects + settings) */
  useEffect(() => {
    if (!user) return;
    let isMounted = true;
    ...
    const loadUserData = async () => {
      ...
      try {
        ...
        setSettings((settingsData as UserSettings) ?? { userId: user.id, messageTemplate: '', searchSources: [], includeWhatsApp: false });

        const hasCompletedInitialTour = localStorage.getItem('tour_initial_' + user.id);
        if (!hasCompletedInitialTour) setActiveTour('initial');
        const hasCompletedCardTour = localStorage.getItem('tour_prospectCard_' + user.id);
        if (hasCompletedCardTour) setHasCompletedProspectTour(true);
      } catch (e) {
  ...
  /** Abre tour após chegar resultado */
  useEffect(() => {
    if (lastProspectsFound.length > 0 && !isSearching && !hasCompletedProspectTour) {
      const t = setTimeout(() => { setActiveTour('prospectCard'); setTourStep(0); setHasCompletedProspectTour(true); }, 500);
      return () => clearTimeout(t);
    }
  }, [lastProspectsFound, isSearching, hasCompletedProspectTour]);

  /** Modal de feedback por inatividade/saída */
  useEffect(() => {
    if (!user || hasShownFeedbackModalThisSession) return;
    ...
    const handleMouseLeave = (e: MouseEvent) => { if (e.clientY <= 0) showModalOnce(); };
    document.addEventListener('mouseleave', handleMouseLeave);
    return () => { clearTimeout(inactivityTimeout); document.removeEventListener('mouseleave', handleMouseLeave); };
  ...
  const handleCloseTour = () => {
    if (user && activeTour) localStorage.setItem('tour_' + activeTour + '_' + user.id, 'true');
    setActiveTour(null); setTourStep(0);
  };
  const handleStartGlobalTour = () => { setView('prospecting'); setActiveTour('initial'); setTourStep(0); };
  const handleStartDatabaseTour = () => { setActiveTour('database'); setTourStep(0); };
  ...
  /** >>> EXECUTA A PROSPECÇÃO (com normalização de respostas) */
  const executeProspecting = useCallback(async () => {
    ...
    try {
      ...
      const prospectsWithDbIds: Prospect[] = Array.isArray(saved) ? saved : saved ? [saved as Prospect] : [];
      ...
      if (signal.aborted) return;
      console.error(e);
      const msg = e instanceof Error ? e.message : 'Ocorreu um erro desconhecido.';
      setError('Falha ao buscar prospects: ' + msg);
    } finally {
      if (!signal.aborted) setIsSearching(false);
    }
  }, [query, searchSources, searchLocation, searchBraziliansAbroad, showToast, user, apiKey, provider]);
  ...
  const handleDeleteProspect = useCallback(async (prospectId: string) => {
    ...
    try {
      ...
      const msg = e instanceof Error ? e.message : 'Ocorreu um erro desconhecido.';
      setError(`Falha ao descartar o prospect: ${msg}`);
    }
  }, [user, showToast]);
  ...
  const handleClearAllProspects = useCallback(async () => {
    ...
    try {
      ...
      const msg = e instanceof Error ? e.message : 'Ocorreu um erro desconhecido.';
      setError('Falha ao limpar a base: ' + msg);
        ...
        {isSearching && (
          <>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 2xl:grid-cols-5 gap-6">
              {Array.from({ length: 10 }).map((_, index) => (<ProspectCardSkeleton key={index} />))}
            </div>
          </>
        )}
      ...
      <div className="min-h-screen text-white flex flex-col items-center p-4 sm:p-6 md:p-8">
        ...
        <div className="fixed bottom-6 right-6 z-40 flex flex-col items-center gap-3">
          <div className={'flex flex-col items-center gap-3 transition-all duration-300 ease-in-out ' + (isFabMenuOpen ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-4 pointer-events-none')}>
            ...
            <button onClick={() => { handleStartGlobalTour(); setIsFabMenuOpen(false); }} className="flex items-center gap-2 bg-gray-700 hover:bg-gray-600 text-white font-semibold py-2 px-4 rounded-full shadow-lg" title="Reiniciar o tour inicial">
