...
import {
  SearchIcon, BrainCircuitIcon, ClipboardIcon, UserGroupIcon,
  ExclamationTriangleIcon, PencilSquareIcon, CheckIcon,
  ArrowRightOnRectangleIcon, BookOpenIcon, PaperAirplaneIcon,
  CheckCircleIcon, ChatBubbleOvalLeftEllipsisIcon, XMarkIcon,
  QuestionMarkCircleIcon, KeyIcon, InstagramIcon
} from './icons';
    ...
    <div className="fixed inset-0 bg-black/60 backdrop-blur-md flex items-center justify-center z-50" aria-modal="true" role="dialog">
      <div className="bg-gray-800 border border-yellow-500/30 rounded-2xl shadow-2xl p-6 sm:p-8 max-w-md w/full m-4 animate-fade-in-up" style={{ animationDuration: '0.3s' }}>
        <div className="text-center">
          ...
          <p className="mt-2 text-gray-300">Iniciar uma nova busca irá limpar os resultados da prospecção atual.</p>
        ...
        <div className="mt-8 grid grid-cols-1 sm:grid-cols-2 gap-4">
          <button onClick={onCancel} className="w-full bg-gray-700 hover:bg-gray-600 text-white font-bold py-3 px-4 rounded-lg transition-all duration-200">Cancelar</button>
          <button onClick={onConfirm} className="w-full bg-red-600 hover:bg-red-700 text-white font-bold py-3 px-4 rounded-lg transition-all duration-200">Continuar e Limpar</button>
        </div>
      </div>
    </div>
  );
...
const initialTourSteps: TourStep[] = [
  { selector: null, title: 'Bem-vindo ao Agente IA!', content: 'Este tour rápido mostrará como usar a ferramenta para encontrar seus primeiros clientes. Vamos começar!' },
  { selector: '[data-tour="search-input"]', title: 'Descreva seu Cliente Ideal', content: 'Digite aqui o perfil do cliente que você procura. Ex: "clínicas de estética em São Paulo".', position: 'bottom' },
  { selector: '[data-tour="sources-container"]', title: 'Fontes de Prospecção', content: 'Escolha onde a IA deve procurar (Google e/ou Instagram).', position: 'bottom' },
  { selector: '[data-tour="location-input"]', title: 'Filtre por Localização (Obrigatório)', content: 'Defina cidade/estado/país para a busca.', position: 'top' },
  { selector: '[data-tour="personalization-container"]', title: 'Personalize a IA', content: 'Conte sobre você para mensagens mais fortes.', position: 'top' },
  { selector: '[data-tour="prospect-button"]', title: 'Comece a Prospectar!', content: 'Clique aqui para iniciar a busca.', position: 'bottom' },
];

const prospectCardTourSteps: TourStep[] = [
  { selector: '[data-tour="prospect-card"]', title: 'Resultados da Prospecção!', content: 'Este é um card de prospect.', position: 'bottom' },
  { selector: '[data-tour="prospect-card-aiscore"]', title: 'Score de Prioridade', content: '0–100: maior = mais oportunidade.', position: 'top' },
  { selector: '[data-tour="prospect-card-analysis"]', title: 'Análise Rápida da IA', content: 'Resumo da presença digital.', position: 'top' },
  { selector: '[data-tour="prospect-card-analysis-breakdown"]', title: 'Análise Detalhada', content: 'Auditoria ponto a ponto.', position: 'top' },
  { selector: '[data-tour="prospect-card-suggestions"]', title: 'Sugestões de Melhoria', content: '3 ideias práticas para abrir conversa.', position: 'top' },
  { selector: '[data-tour="prospect-card-generate-message"]', title: 'Gerador de Mensagens', content: 'Sequência de 3 mensagens de WhatsApp.', position: 'bottom' },
  { selector: '[data-tour="prospect-card-whatsapp"]', title: 'Contato Direto', content: 'Abre o WhatsApp com a mensagem inicial.', position: 'top' },
  { selector: '[data-tour="nav-tabs"]', title: 'Próximo Passo: Sua Base', content: 'Tudo vai para a Base de Prospectados.', position: 'bottom' },
];

const databaseTourSteps: TourStep[] = [
  { selector: '[data-tour="kanban-board"]', title: 'Sua Base de Prospectados', content: 'Seu funil de vendas (Kanban).', position: 'bottom' },
  { selector: '[data-tour="kanban-column-new"]', title: 'Colunas do Funil', content: 'Personalizáveis do Novo ao Fechado.', position: 'bottom' },
  { selector: '[data-tour="kanban-card"]', title: 'Cards de Prospect', content: 'Arraste/solte para mudar estágio.', position: 'right' },
  { selector: null, title: 'Explore os Detalhes', content: 'Abra um card para ver tudo e agir.' },
];
...
const FeedbackForm: React.FC<{ onShowToast: (message: string) => void; userId: string; }> = ({ onShowToast, userId }) => {
  ...
  const handleSuggestionSubmit = async () => {
    if (!suggestionText.trim()) { setSubmitError('Por favor, escreva uma sugestão.'); return; }
    setSubmitError(null); setIsSubmitting(true);
    const { success, error } = await submitSuggestion({ suggestion: suggestionText, userId });
    setIsSubmitting(false);
    if (success) { setIsSubmitted(true); onShowToast('Sugestão enviada, obrigado!'); }
    else { setSubmitError(error || 'Ocorreu um erro ao enviar. Tente novamente.'); }
  };
  ...
  return (
    <div className="space-y-3">
      ...
      <textarea
        ...
        onChange={(e) => { setSuggestionText(e.target.value); if (submitError) setSubmitError(null); }}
        rows={4}
      ...
      >
        {isSubmitting ? (<><div className="animate-spin rounded-full h-5 w-5 border-b-2 border-white"></div><span>Enviando...</span></>) : (<><PaperAirplaneIcon className="h-5 w-5" /><span>Enviar Sugestão</span></>)}
      </button>
    </div>
  )
}
...
export const ProspectingTool: React.FC = () => {
  ...
  const [settings, setSettings] = useState<UserSettings>({ messageTemplate: '', kanbanColumnTitles: { new: 'Novos', contacted: 'Contatados', negotiating: 'Em Negociação', won: 'Contrato fechado' } });
  ...
  const handleLocationChange = useCallback((v: string) => { setSearchLocation(v); if (error) setError(null); }, [error]);
  ...
  const handleSourceChange = (source: SearchSource) => {
    ...
    setSearchSources(prev => {
      ...
      if (ns.has(source)) ns.delete(source); else ns.add(source);
      return ns;
    });
  };

  /* Executa a prospecção */
  const executeProspecting = useCallback(async () => {
    ...
    const finalQuery = searchBraziliansAbroad
      ? `${query} (foco em negócios e empreendedores brasileiros na localização especificada)`
      : query;

    try {
      const resp = await findProspects(finalQuery, Array.from(searchSources), searchLocation, { apiKey, provider });
      const foundProspectsRaw = resp?.prospects ?? [];
      const foundProspects: Prospect[] = Array.isArray(foundProspectsRaw) ? foundProspectsRaw : [];

      if (signal.aborted) return;

      if (foundProspects.length === 0) {
        setError('Nenhum prospect novo encontrado. Tente refinar sua busca.');
        setLastProspectsFound([]);
        showToast('Nenhum prospect novo encontrado.');
        return;
      }

      const prospectsWithUserAndSource = foundProspects.map(p => ({
        ...p,
        user_id: user.id,
        foundOn: Array.from(searchSources),
      }));

      // salva; pode retornar [] caso a policy bloqueie SELECT no retorno
      const saved = await dataService.addProspects(user.id, prospectsWithUserAndSource);
      const savedArray: Prospect[] = Array.isArray(saved) ? saved : (saved ? [saved as Prospect] : []);

      // fallback para exibição imediata
      const toDisplay = savedArray.length > 0 ? savedArray : prospectsWithUserAndSource;

      setLastProspectsFound(toDisplay);
      if (savedArray.length > 0) {
        setProspects(prev => [...prev, ...savedArray]);
      }
      showToast(`${toDisplay.length} novo(s) prospect(s) adicionado(s).`);
    } catch (e: any) {
      if (signal.aborted) return;
      console.error(e);
      setError('Falha ao buscar prospects: ' + (e?.message ?? 'Erro desconhecido.'));
    } finally {
      if (!signal.aborted) setIsSearching(false);
    }
  }, [query, searchSources, searchLocation, searchBraziliansAbroad, user, apiKey, provider]);

  const handleProspect = useCallback(() => {
    setError(null); setSourceError(null);
    if (searchSources.size === 0) { setSourceError('Selecione ao menos uma fonte para continuar.'); return; }
    if (!query.trim()) { setError('Por favor, insira um critério de busca para continuar.'); return; }
    if (!searchLocation.trim()) { setError('O campo Localização é obrigatório.'); return; }
    if (!apiKey || !provider) { setError('Por favor, configure sua chave de API do Gemini para continuar.'); setIsApiKeyModalOpen(true); return; }
  ...
  const handleConfirmProspecting = useCallback(() => { setIsWarningModalOpen(false); executeProspecting(); }, [executeProspecting]);
  ...
  const handleKeyDown = (e: React.KeyboardEvent<HTMLInputElement>) => {
    if (e.key === 'Enter' && !isSearching) handleProspect();
  };
  ...
  const handleDeleteProspect = useCallback(async (prospectId: string) => {
    ...
    try {
      ...
      setError(`Falha ao descartar o prospect: ${e?.message ?? 'Erro desconhecido.'}`);
    }
  }, [user, showToast]);
  ...
  const handleClearAllProspects = useCallback(async () => {
    ...
    try {
      ...
      setError('Falha ao limpar a base: ' + (e?.message ?? 'Erro desconhecido.'));
    ...
    <>
      <div className="sticky top-4 z-10 bg-gray-900/60 backdrop-blur-xl p-4 rounded-xl border border-gray-700/80 shadow-2xl shadow-blue-900/10">
        <div className="flex flex-col sm:flex-row gap-3">
          ...
          {isSearching ? (
            <div className="flex items-center justify-center gap-3 bg-gray-700 text-gray-400 font-bold py-3 px-4 rounded-lg">
              <div className="animate-spin rounded-full h-5 w-5 border-b-2 border-white"></div><span>Buscando...</span>
              <button onClick={handleCancelSearch} className="bg-red-600 hover:bg-red-700 text-white font-bold py-1 px-3 rounded-md text-sm -my-1 -mr-1">Cancelar</button>
            </div>
          ) : (
            <button data-tour="prospect-button" onClick={handleProspect} disabled={!query.trim() || !searchLocation.trim()} className="flex items-center justify-center gap-2 bg-blue-600 hover:bg-blue-700 disabled:bg-gray-700 disabled:text-gray-400 disabled:cursor-not-allowed text-white font-bold py-3 px-6 rounded-lg transition-all duration-200 shadow-md hover:shadow-lg focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-offset-gray-900 focus:ring-blue-500">
              <SearchIcon className="h-5 w-5" /><span>Prospectar</span>
            </button>
          )}
        </div>
        <div className="mt-4 grid grid-cols-1 md:grid-cols-2 gap-4">
          ...
          <div>
            <div data-tour="location-input">
              ...
              <LocationAutocomplete value={searchLocation} onChange={handleLocationChange} disabled={isSearching} />
            </div>
            <div className="mt-3">
              <label htmlFor="brazilians-abroad" className="flex items-center gap-2.5 cursor-pointer group">
                <input id="brazilians-abroad" type="checkbox" checked={searchBraziliansAbroad} onChange={(e) => setSearchBraziliansAbroad(e.target.checked)} className="h-5 w-5 rounded bg-gray-700 border-gray-600 text-blue-600 focus:ring-2 focus:ring-blue-500 focus:ring-offset-gray-900 transition-colors" />
                <span className="text-sm font-medium text-gray-300 group-hover:text-white transition-colors">Focar em empreendedores brasileiros no exterior</span>
              </label>
            </div>
          </div>
        </div>
        <div data-tour="personalization-container" className="mt-4">
          <label htmlFor="message-template" className="block text-sm font-medium text-gray-300 mb-2">Personalização da IA (Opcional)</label>
          {isTemplateExpanded ? (
            <div className="relative">
              ...
              <button onClick={() => setIsTemplateExpanded(false)} className="absolute top-2 right-2 flex items-center gap-1.5 bg-gray-700 hover:bg-gray-600 text-white font-semibold py-1 px-3 rounded-lg text-sm"><CheckIcon className="h-4 w-4" />Fechar</button>
            </div>
          ) : (
            <div className="flex justify-between items-center bg-gray-800 border border-dashed border-gray-600 rounded-lg p-3 group">
              <p className="text-gray-400 text-sm truncate pr-4">{settings?.messageTemplate ? settings.messageTemplate.split('\n')[0] : 'Nenhuma personalização definida'}</p>
              <button onClick={() => setIsTemplateExpanded(true)} className="flex-shrink-0 flex items-center gap-2 text-blue-400 font-semibold text-sm hover:text-blue-300"><PencilSquareIcon className="h-4 w-4" /><span>{settings?.messageTemplate ? 'Editar' : 'Adicionar'}</span></button>
            </div>
          )}
        ...
        {isSearching && (
          <>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 2xl:grid-cols-5 gap-6">
              {Array.from({ length: 10 }).map((_, idx) => (<ProspectCardSkeleton key={idx} />))}
            </div>
          </>
        )}
  ...
  const NavButton: React.FC<{ currentView: View; targetView: View; setView: (v: View) => void; children: React.ReactNode }> = ({ currentView, targetView, setView, children }) => {
    const isActive = currentView === targetView;
    return (
      <button onClick={() => setView(targetView)} className={'flex items-center gap-2 px-4 py-2 text-base font-semibold rounded-lg transition-all duration-200 ' + (isActive ? 'bg-blue-600 text-white shadow-md' : 'text-gray-400 hover:bg-gray-700/50 hover:text-white')}>
        {children}
      </button>
    );
  };
      ...
      <div className="min-h-screen text-white flex flex-col items-center p-4 sm:p-6 md:p-8">
        <div className="w-full max-w-7xl mx-auto flex flex-col flex-grow">
          <div className="flex-grow">
            <header className="relative text-center mb-8">
              <div className="flex items-center justify-center gap-4 mb-2">
                ...
                <h1 className="text-4xl sm:text-5xl font-bold bg-gradient-to-r from-blue-400 to-cyan-300 text-transparent bg-clip-text">Agente de Prospecção IA</h1>
              </div>
              ...
              <div className="absolute top-0 right-0 flex items-center gap-2">
                <button onClick={() => setIsApiKeyModalOpen(true)} className="flex items-center gap-2 bg-gray-800 hover:bg-blue-800/50 text-gray-300 hover:text-blue-300 font-semibold py-2 px-4 rounded-lg transition-all duration-200 border border-gray-700 hover:border-blue-700/50" title="Configurar sua API Key do Gemini">
                  <KeyIcon className="h-5 w-5" /><span className="hidden sm:inline">Configurar IA</span>
                </button>
                <button onClick={signOut} className="flex items-center gap-2 bg-gray-800 hover:bg-red-800/50 text-gray-300 hover:text-red-300 font-semibold py-2 px-4 rounded-lg transition-all duration-200 border border-gray-700 hover:border-red-700/50" title="Sair da sua conta">
                  <ArrowRightOnRectangleIcon className="h-5 w-5" /><span className="hidden sm:inline">Sair</span>
                </button>
              </div>
            </header>
            ...
            <main className="w-full">
              {view === 'prospecting' ? renderProspectingView() : (
                <DatabaseView
                  ...
                  onStartTour={() => setActiveTour('database')}
          ...
          <footer className="text-center mt-12 py-4 text-gray-500 text-sm border-t border-gray-800">
            <p>Agente de Prospecção IA - Versão {APP_VERSION}</p>
          </footer>
        </div>
        ...
        {isFabMenuOpen && <div className="fixed inset-0 z-30" onClick={() => setIsFabMenuOpen(false)} />}
        <div className="fixed bottom-6 right-6 z-40 flex flex-col items-center gap-3">
          <div className={'flex flex-col items-center gap-3 transition-all duration-300 ease-in-out ' + (isFabMenuOpen ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-4 pointer-events-none')}>
            <button onClick={() => { setIsManualFeedbackModalOpen(true); setIsFabMenuOpen(false); }} className="flex items-center gap-2 bg-gray-700 hover:bg-gray-600 text-white font-semibold py-2 px-4 rounded-full shadow-lg" title="Enviar feedback ou sugestão">
              <ChatBubbleOvalLeftEllipsisIcon className="h-6 w-6" /><span className="text-sm">Feedback</span>
            </button>
            <button onClick={() => { setActiveTour('initial'); setIsFabMenuOpen(false); }} className="flex items-center gap-2 bg-gray-700 hover:bg-gray-600 text-white font-semibold py-2 px-4 rounded-full shadow-lg" title="Reiniciar o tour inicial">
              <BookOpenIcon className="h-6 w-6" /><span className="text-sm">Ver Tutorial</span>
            </button>
          </div>
          <button onClick={() => setIsFabMenuOpen(!isFabMenuOpen)} className={'bg-blue-600 hover:bg-blue-700 text-white rounded-full p-4 shadow-2xl transition-transform duration-300 transform ' + (isFabMenuOpen ? 'rotate-45' : 'hover:scale-110')} aria-label={isFabMenuOpen ? 'Fechar menu de ajuda' : 'Abrir menu de ajuda'} aria-expanded={isFabMenuOpen}>
        ...
        {activeTour && <InteractiveTour steps={currentTourSteps} stepIndex={tourStep} onClose={() => setActiveTour(null)} onNext={() => setTourStep(s => s + 1)} onPrev={() => setTourStep(s => s - 1)} />}
