...
import { BrainCircuitIcon, SearchIcon, SparklesIcon, ArrowRightIcon, UserGroupIcon, ChatBubbleLeftRightIcon, TagIcon } from './components/icons';
...
const LandingPage: React.FC<{ onNavigate: (page: Page) => void }> = ({ onNavigate }) => (
  <div className="min-h-screen text-white bg-grid-pattern">
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-16 sm:py-24 animate-fade-in-up" style={{ animationDuration: '0.8s' }}>
      <header className="text-center mb-16">
        ...
        <p className="mt-6 max-w-3xl mx-auto text-lg sm:text-xl text-gray-300">
          Pare de procurar clientes manualmente. Encontre leads qualificados, analise sua presença digital e crie mensagens de contato que convertem, tudo com o poder da IA.
        </p>
        <div className="mt-10">
          <button
      ...
      <main>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
          <FeatureCard icon={<SearchIcon className="h-6 w-6 text-blue-300" />} title="Prospecção Global Inteligente" description="Encontre leads qualificados no Brasil e no mundo, prospectando por múltiplos canais como Google e Instagram, para fechar contratos em Real, Dólar e/ ou Euro." />
          <FeatureCard icon={<BrainCircuitIcon className="h-6 w-6 text-blue-300" />} title="Análise Completa com Direcionamento" description="Receba uma auditoria profunda sobre cada lead, com insights estratégicos e recomendações práticas de próximos passos para avançar no funil." />
          <FeatureCard icon={<SparklesIcon className="h-6 w-6 text-blue-300" />} title="Mensagens que Convertem" description="Obtenha sequências prontas de mensagens personalizadas: saudação, conexão com a dor e fechamento, criadas pela IA com base no lead e no seu perfil profissional." />
          <FeatureCard icon={<UserGroupIcon className="h-6 w-6 text-blue-300" />} title="Gestão de Leads Simplificada" description="Organize contatos em uma base nativa estilo Kanban, acompanhe o ciclo de cada prospect e tenha clareza total sobre onde atuar." />
          <FeatureCard icon={<ChatBubbleLeftRightIcon className="h-6 w-6 text-blue-300" />} title="Quebra de Objeções em Tempo Real" description="Cole a conversa com o prospect e receba respostas práticas da IA para contornar objeções e transformar oportunidades em contratos." />
          <FeatureCard icon={<TagIcon className="h-6 w-6 text-blue-300" />} title="Insights Estratégicos para Abordagem" description="Descubra os pontos fortes e fracos da presença digital do lead e use essas informações para criar uma abordagem muito mais persuasiva." />
        </div>
      </main>
...
// Prioriza reset-password mesmo com sessão temporária
