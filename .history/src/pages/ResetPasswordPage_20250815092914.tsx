...
export default function ResetPasswordPage({ onNavigate }: Props) {
  ...
  // Checa se a sessão de recuperação está ativa
  useEffect(() => {
    let mounted = true;
    ...
    // e se algo mudar (por ex, Supabase finalizar parsing do hash)
  ...
  const onSubmit = async (e: React.FormEvent) => {
    ...
    if (!hasRecoverySession) {
      setErr('Sessão de autenticação ausente. Abra o link de redefinição de senha a partir do e-mail novamente.');
      return;
    }
        ...
        <form onSubmit={onSubmit} className="space-y-3">
          ...
          {!hasRecoverySession && (
            <p className="text-red-400 text-sm">
              Sessão de autenticação ausente. Reabra o link de redefinição de senha a partir do e-mail.
            </p>
          )}
