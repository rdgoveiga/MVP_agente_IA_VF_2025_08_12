...
export const AuthProvider: React.FC<{
  ...
  // Se voltar com #...type=recovery, manda p/ tela de reset (sem limpar sessão)
  useEffect(() => {
    const hash = window.location.hash || '';
    if (hash.includes('type=recovery')) {
      isRecoveryRef.current = true;
      navigate('reset-password');
      // o initialLoading será liberado no listener abaixo
    }
  }, [navigate]);
  ...
  // IMPORTANTE: NÃO colocar "#..." no redirectTo. Deixe só a origem.
  const requestPasswordReset = async (email: string) => {
    setLoading(true);
    const { error } = await supabase.auth.resetPasswordForEmail(email, {
      redirectTo: window.location.origin, // Supabase acrescenta #access_token=...&type=recovery
    });
    setLoading(false);
    return handleError(error as any);
  };
