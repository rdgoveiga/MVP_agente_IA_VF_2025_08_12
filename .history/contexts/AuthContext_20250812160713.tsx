...
// Esse navigate vem de fora (App), vamos receber ele via props no AuthProvider
...
export const AuthProvider: React.FC<{ children: ReactNode; navigate: (page: string) => void }> = ({ children, navigate }) => {
  ...
  useEffect(() => {
    const { data: { subscription } } = supabase.auth.onAuthStateChange((event, session) => {
      if (event === 'PASSWORD_RECOVERY') {
        // Troca de tela interna, sem recarregar o app
  ...
  const updateUserPlan = async (plan: 'lifetime') => {
    ...
    setLoading(false);
    return error ? handleError(error) : { error: null };
  };
