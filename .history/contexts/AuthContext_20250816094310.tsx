...
const APP_URL = SITE_URL; // já vem de VITE_SITE_URL em produção
...
export const AuthProvider: React.FC<{ children: ReactNode }> = ({ children }) => {
  ...
  const signUp = async (
    ...
    // <<< IMPORTANTE: força aprovação manual do admin
    const baseMeta = { full_name, whatsapp, is_validated: false };

    const { data, error } = await supabase.auth.signUp({
      email,
      password,
      options: {
        data: baseMeta, // vai para auth.users.raw_user_meta_data
        emailRedirectTo: `${APP_URL}/#login`, // após confirmar e-mail volta pro login (SPA)
        autoSignIn: false, // não loga automaticamente após confirmar
      },
    });

    // Reflete imediatamente no painel (às vezes o meta demora a aparecer)
    if (!error && data.user) {
      await supabase.auth.updateUser({ data: baseMeta });
    }
  ...
  const login = async (email: string, password: string) => {
    ...
    // Se o admin ainda não liberou (is_validated !== true), bloqueia o acesso
    const isValidated = (data.user as any)?.user_metadata?.is_validated;
    if (isValidated !== true) {
      await supabase.auth.signOut();
      return handleError({
        message:
          'Conta aguardando aprovação do administrador. Você será notificado quando for liberada.',
      });
    }
